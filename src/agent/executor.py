...
3. 【可选】对陌生/难判断的用户，调用 find_similar_users 从 L2 FAISS 检索画像相似的历史用户，辅助推理偏好
4. 对每个用户调用 match_user_content 找到最佳匹配
5. 对每个用户调用 generate_recommendation 生成最终推荐
...
- **如果上面列出了学到的策略，你必须先阅读策略的"执行步骤"和"内容方向"，然后按策略的指导筛选和匹配内容**
- 推荐理由要具体，说明用户兴趣与内容的关联，并**显式引用你应用了哪条策略**（例如"应用《策略名》的执行步骤 2"）
- 策略是过去反思学到的经验，不遵循它们就是浪费以前的学习成本"""
...
class AdExecutor:
    ...
    def __init__(
        ...
        # When user_profile_store is provided, get_all_tools injects the
        # `find_similar_users` tool so the ReAct agent can actually query L2.
        self.tools = get_all_tools(user_profile_store=user_profile_store)
        self.graph = self._build_graph()
        # Record which strategies were injected in the last execute() call — useful
        # for debugging evolution rounds and for a future Verifier to correlate
        # metrics with strategies used.
        self.last_applied_strategies: list[str] = []
    ...
    def _load_strategies(self, scenario: str) -> str:
        ...
        strategies = self._retrieve_relevant_strategies(scenario, k=3)
        if not strategies:
            # Zero retrieval hits but lib is non-empty — show names as hints.
            index = self.strategy_lib.list_all()
            return "可用策略：\n" + "\n".join(
                f"- {s['name']}: {s['applicable_scenario']}" for s in index[:5]
            )

        self.last_applied_strategies = [s.strategy_id for s in strategies]
        ...
        for s in strategies:
            ...
            parts.append(
                ...
                f"目标人群: {s.target_audience}\n内容方向: {s.content_direction}\n"
                f"执行步骤:\n{steps}\n预期效果: {s.expected_effect}"
            )
        return "\n\n".join(parts)

    def _retrieve_relevant_strategies(self, scenario: str, k: int = 3):
        """Retrieve top-k strategies: semantic search if FAISS is available,
        keyword search otherwise. Falls back to most recent strategies."""
        query = scenario or "推荐内容给用户，提升点击率和完播率"

        if self.strategy_lib.has_vector_index:
            hits = self.strategy_lib.semantic_search(query, k=k)
            return [s for s, _ in hits]

        if scenario:
            return self.strategy_lib.search(scenario)[:k]

        # No FAISS, no scenario — take the most recent strategies.
        index = self.strategy_lib.list_all()[-k:]
        return [self.strategy_lib.get(e["strategy_id"]) for e in index]
